...
That being said, the follow installation instructions are with the Raspberry Pi 3 (probably 2 also) in mind.
...
The following commands can be run using `pete [command]`:

- `install` - Goes through the installation process. If installation has already taken place, it will ask if you'd like to overwrite your settings.
- `tv-setup` - Goes through each show in your TMdb TV watchlist and sets up the season and episode you wish to start downloading and the desired quality (720p or HDTV). You can run this at any time to overwrite or skip shows you don't need to overwrite.
