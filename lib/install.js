...
module.exports = () => {
  ...
  inquirer.prompt(prompts.begin).then(answer => {
    ...
    if (!_.isEmpty(config.tmdb.apiKey) && !_.isEmpty(config.tmdb.sessionId)) {
      return config.save()
    }
  ...
  .then(() => models.haveExistingItems()) // Checks to see if there are existing items in the database
  .then(notEmpty => {
    if (!notEmpty) {
      return
    }
