...
const addTorrent = (self, magnet, directory, finalName) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    client.add(magnet, torrent => {
      const torrentPath = p.join(config.torrentFiles, `${finalName}.torrent`)
      ...
      fs.writeFile(torrentPath, torrent.torrentFile, err => {
        ...
        self.addFile(torrentPath, { 'download-dir': directory }, (err, torrent) => {
          ...
          self.get(torrent.id, (err, torrents) => {
            ...
            const files = torrents.torrents[0].files
            ...
            async.eachOfSeries(torrents.torrents[0].files, (file, index, callback) => {
              ...
              if (
                file.name.indexOf('/') === -1 ||
                (file.name.search(/.(mkv|avi|mp4|mov|srt|sub)$/gi) !== -1 && file.name.search(/(sample|rarbg\.com)/gi) === -1)
              ) {
                newName = finalName + p.extname(file.name)
              }

              self.rename(torrentInfo.id, file.name, newName, callback)
            }, err => {
              if (err) {
                return reject(err)
              }
              ...
              if (files[0].name.indexOf('/') !== -1) {
                self.rename(torrentInfo.id, p.dirname(files[0].name), finalName, err => {
