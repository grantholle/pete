...
const config = require('./config'),
      ...
      EPISODE_FAILURE_MAX = 24, // An entire day at checking once an hour

      getAirDate = dateStr => {
        // It wil get parsed as as the air date at midnight (of that day) in EST
        // so by adding a day, we'll more than likely be checking only a
        // few hours after it truly aired, making for a more realistic time to search
        return moment(`${dateStr} -05:00`, 'YYYY-MM-DD ZZ').add(1, 'days')
      },

      processEpisode = (item, cb) => {
        ...
        searchForShowTorrent(copy, (err, url) => {
          ...
          if (url) {
            addToTransmission(copy, url, cb)
          } else {
            winston.info(`No torrents could be found for ${item.name} ${label(item.season, item.episode)}`)

            if (now.diff(item.airDate, 'days') > 2) {
              mediaDb.incrementAttempt(copy, cb)
            } else {
              cb(null)
            }
          }
      ...
      // Searches eztv and rarbg as a fallback
      ...
      searchAlternativeEztvQuality = (show, cb) => {
        ...
        if (c && c.episodes[show.season.toString()] && c.episodes[show.season.toString()][show.episode.toString()]) {
      ...
      searchRarbg = (show, cb) => {
        let searchStr = `${show.name.replace(/'/gi, '')} ${label(show.season, show.episode)}`
      ...
      addToTransmission = (show, magnetUrl, callback) => {
        ...
        transmission.addUrl(magnetUrl, { 'download-dir': show.downloadDir }, (err, torrent) => {
          ...
          show.downloadDir = p.join(show.downloadDir, torrent.name)
          notify(`Started download for ${show.name} ${label(show.season, show.episode)} - ${show.epName}`)
          winston.info(`Added torrent for ${show.name} ${label(show.season, show.episode)} - ${show.epName}`)
          show.transmission_id = torrent.id
          show.transmission_name = torrent.name

          mediaDb.addEpisode(show, callback)
        })
      }
...
module.exports = () => {
  ...
  mediaDb.getShowsAndDownloads((err, showsConfig) => {
    moviedb.accountTvWatchlist({ id: '{account_id}' }, (err, watchlistResponse) => {
      ...
      eachOfSeries(watchlistResponse.results, (watchlistShow, key, watchlistIterationCb) => {
        ...
        /**
         * There needs to be a flag set for the date of the show
         *
         * When we're iterating through season episodes,
         * if the episode date is in the future, we need to
         * store that date and only retrieve that show information
         * if today is after that date flag
         * this will save on calls to tmdb's api and make it run more efficiently
         *
         * This means we'll have to retrieve the shows from the local db first
         */
        ...
        moviedb.tvInfo({ id: watchlistShow.id }, (err, showResponse) => {
          ...
          if (showsConfig.hasOwnProperty(watchlistShow.id.toString())) {
            ...
            const applicableSeasons = showResponse.seasons
              .filter(season => ((getAirDate(season.air_date) < now) && (parseInt(show.start_season, 10) <= season.season_number)))
              .sort((a, b) => a - b)
            ...
            eachOfSeries(applicableSeasons, (season, index, seasonCallback) => {
              ...
              if (season.episode_count !== episodes.length) {
                  ...
                  eachOf(seasonResponse.episodes, (episode, i, episodeCb) => {
                    ...
                    if ((isBaseSeason && parseInt(show.start_episode, 10) <= episode.episode_number && episodes.indexOf(episode.episode_number) === -1) || (episodes.indexOf(episode.episode_number) === -1)) {
                      ...
                      if (parseInt(show.start_episode, 10) <= episode.episode_number && episodes.indexOf(episode.episode_number) === -1) {
                        ...
                        mediaDb.getShowAttempts(item, (err, count) => {
                          if (count > EPISODE_FAILURE_MAX) {
                            mediaDb.addEpisode(item, err => {
                              if (err)
                                return winston.error(err)
                            ...
                            processEpisode(item, err => {
                              if (err)
                                winston.error(err)
                              else
                                episodesGotThisSeason++
                ...
                // If we're at this point, we've already downloaded all the episodes in this season
                // Increment the start_season of the show to prevent checking this season again
                // Check the index to make sure it's not the most recent season
                if (index !== applicableSeasons.length - 1) {
                  mediaDb.incrementStartSeason(watchlistShow.id, err => {
                    if (err)
                      winston.error(err)

                    winston.info(`All episodes in season ${season.season_number} for ${showResponse.name} have been downloaded. Set start season to ${season.season_number + 1}`)
                    seasonCallback(null)
                  })
                } else {
                  seasonCallback(null)
            ...
            eachOfSeries(showResponse.seasons, (season, index, seasonIterationCb) => {
              ...
              if (getAirDate(season.air_date) > now) {
