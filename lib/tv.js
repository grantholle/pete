...
const config = require('./config'),
      ...
      searchForShowTorrent = (show, cb) => {
        const searchEztv = data => {
          let quality = show.quality.toUpperCase() === 'HDTV' ? '480p' : show.quality
          // require('jsonfile').writeFile(`${sanitize(show.name)}-${show.season}-${show.episode}-ez.json`, data)
      ...
      searchRargb = (show, cb) => {
        let searchStr = `${show.name} S${pad(show.season)}E${pad(show.episode)}`
        ...
        rargb.search({
          ...
          if (results.torrent_results && results.torrent_results.length > 0) {
            ...
            if (desiredQuality) {
              ...
              if (desiredQuality) {
                winston.info(`Episode found! Adding ${show.name} for season ${show.season} episode ${show.episode} [${show.quality.toUpperCase() === 'HDTV' ? '720p' : 'HDTV'}]`)
                cb(null, desiredQuality.download)
              } else {
                cb(`No torrents could be found for ${show.name} season ${show.season} episode ${show.episode}`, {})
              }
            }
          }
      ...
      addToTransmission = (show, magnetUrl, callback) => {
        ...
        transmission.addUrl(magnetUrl, { 'download-dir': show.downloadDir }, (err, torrent) => {
          ...
          winston.info(`Added torrent for ${show.name} S${pad(show.season)}E${pad(show.episode)} - ${show.epName}`)
...
showsDb.getShowsAndDownloads((err, showsConfig) => {
  moviedb.accountTvWatchlist({ id: '{account_id}' }, (err, watchlistResponse) => {
    ...
    eachOfSeries(watchlistResponse.results, (watchlistShow, key, watchlistIterationCb) => {
            ...
            if (season.episode_count !== episodes.length) {
              ...
              moviedb.tvSeasonInfo({ id: showResponse.id, season_number: season.season_number }, (err, seasonResponse) => {
                ...
                eachOf(seasonResponse.episodes, (episode, i, episodeCb) => {
                  ...
                  const seasonDownloads = episodesToDownload.filter(item => item.tmdb_id === watchlistShow.id && item.season === season.season_number)
      ...
      //   })
      // }, () => { // all items iterated
        winston.info('Finished tv script')
      // })
