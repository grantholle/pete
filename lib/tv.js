      ...
      searchRargb = (show, cb) => {
        ...
        rargb.search({
          ...
          if (results.torrent_results && results.torrent_results.length > 0) {
            ...
            cb(`Nothing found for ${show.name}, season ${show.season}, episode ${show.episode} [${show.quality}]`, {})
          }
          ...
          moviedb.tvSeasonInfo({ id: showResponse.id, season_number: newShow.season }, (err, seasonResponse) => {
            ...
            eztv.searchCache(newShow.name).then(eztvInfo => {
              ...
              eachOfSeries(seasonResponse.episodes, (episode, key, episodeCallback) => {
                newShow.episode = episode.episode_number

                // get all episodes of the season (not recurring show) or get latest episode (recurring show)
                // if (!isRecurringShow) {
                ...
                // If the air date is past today, don't try to add it
                if (moment(episode.air_date, 'YYYY-MM-DD') > today) {
                  const prevEp = seasonResponse.episodes.find(ele => {
                    return ele.episode_number === episode.episode_number - 1
                  })

                  if (prevEp)
                    newShow.episode = prevEp.episode_number

                  return episodeCallback(true)
                }

                episodeCallback(null)
              }, found => {
                if (found) {
                  winston.info(`Queueing ${newShow.name} season ${newShow.season}, episode ${newShow.episode}`)
                  episodesToDownload.push({
                    tmdb_id: watchlistShow.id,
                    name: newShow.name,
                    season: newShow.season,
