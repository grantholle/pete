...
/**
 *
 * @param {Integer} originalName The name of the torrent
 */
const getTorrent = originalName => {
  return loadDatabase().then(() => {
    return torrents.findOne({ originalName })
  })
}
 ...
 *
 * @param {String} originalName The original name of the torrent
 * @param {String} newName The reformatted name of the torrent
 * @returns {Promise}
 */
const saveTorrent = (originalName, newName) => {
  return new Promise(resolve => {
    loadDatabase().then(() => {
      torrents.insert({ originalName, newName })
      return saveDatabase()
    }).then(() => resolve())
  })
}
 ...
 *
 * @param {Integer} originalName The transmission id of the torrent to delete
 * @returns {Promise}
 */
const deleteTorrent = originalName => {
  return loadDatabase().then(() => {
    torrents.removeWhere({ originalName })
