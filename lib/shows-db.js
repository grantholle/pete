...
const os = require('os'),
      ...
      obj = {
        ...
        getAttempts(show, cb) {
          this.db.get('select attempts from attempts where tmdb_id = ? and season = ? and episode = ?', [show.tmdb_id, show.season, show.episode], (err, count) => {
            if (err)
              return cb(err)

            cb(null, count ? parseInt(count, 10) : 0)
          })
        },
        getShows(cb) {
