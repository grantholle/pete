...
const fs = require('fs-extra'),
      p = require('path'),
      os = require('os'),
      workingDirectory = p.join(__dirname, '..'),
      startScript = p.join(workingDirectory, 'scripts', 'start.js'),
      winston = require('./logger'),
      inquirer = require('inquirer'),
      config = require('./config'),
      exec = require('child_process').exec,
      prompts = require('./prompts').installService,
      configServiceFilePath = p.join(config.dir.home, 'pete.service'),
      colors = require('colors')

module.exports = () => {
  if (process.platform.toLowerCase() !== 'linux') {
    return winston.info(`Your current platform (${process.platform}) does not support the use of systemd service files. Please research ways of running a node script as a process in your current platform.`)
  }
