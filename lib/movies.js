...
const config = require('./config'),
      ...
      searchRargb = (movie, quality, cb) => {
        ...
        rargb.search({
          ...
          if (err.error_code === 20 && !searchedAlready && config.movies.fallback) {
            ...
            winston.info(`Nothing found for ${movie.title} (${quality}) on RARGB. Attempting search in ${alt}`)
            return searchRargb(movie, alt, cb)
          } else if (err.error_code === 20) {
            winston.info(`Nothing found for ${movie.title} on RARGB.`)
      ...
      addToTransmission = (movie, magnetUrl, callback) => {
        ...
        transmission.addUrl(magnetUrl, { 'download-dir': config.dir.movies }, (err, torrent) => {
          ...
          mediaDb.addMovieDownload({ tmdb_id: movie.id, transmission_id: torrent.id, title: title, dir: p.join(config.dir.movies, torrent.name) })
