...
module.exports = config => {
  ...
  inquirer.prompt(prompts.continue).then(answer => {
    ...
    fs.pathExists(configServiceFilePath).then(existingServiceFile => {
      ...
      exec(`sudo cp ${configServiceFilePath} /lib/systemd/system/pete.service && sudo systemctl daemon-reload && sudo systemctl enable pete && sudo systemctl start pete`, (err, stdout) => {
        if (err) {
