...
module.exports = config => {
  ...
  database.loadDatabase().then(() => {
    return inquirer.prompt(prompts.continue)
  }).then(answer => {
    if (!answer.continue) {
    ...
    eachSeries(showsToSetUp, (show, callback) => {
      const setup = answer => {
        ...
        moviedb.tvInfo(show.id).then(showInfo => {
          ...
          configuration.imdb_id = imdbId

          return database.findOrCreateShow(show)
        }).then(currentShow => {
          return database.saveShow(_.assign(currentShow, configuration))
        }).then(() => {
