...
const os = require('os'),
      ...
      obj = {
        ...
        addEpisode(e, cb) {
          this.db.run('insert into downloads (tmdb_id, show, name, episode, season, transmission_id, download_dir) values (?, ?, ?, ?, ?, ?, ?)', [e.tmdb_id, e.name, e.epName, e.episode, e.season, e.transmissionId, e.downloadDir], cb)
        },
        addMovieDownload(movie) {
          this.db.run('insert into downloads (tmdb_id, name, transmission_id, download_dir) values (?, ?, ?, ?)', [movie.tmdb_id, movie.title, movie.transmission_id, movie.dir])
        },
        getInprogress(tmdb_id, cb) {
          this.db.all('select * from downloads where renamed = false', [], cb)
        },
        updateRenamed(id, cb) {
          this.db.run('update downloads set renamed = true where id = ?', [id], cb)
        },
        ...
        createDbs() {
          const createShows = 'create table if not exists shows (id integer primary key, tmdb_id integer unique, name varchar(255), start_season integer, start_episode integer, desired_quality varchar(10), eztv text)',
                createDownloads = 'create table if not exists downloads (id integer primary key, tmdb_id integer, show varchar(255), name varchar(255), episode integer, season integer, transmission_id integer, download_dir varchar(255), renamed boolean default false, foreign key (tmdb_id) references shows(tmdb_id))',
