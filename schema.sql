...
CREATE TABLE IF NOT EXISTS downloads (id integer primary key, tmdb_id integer, show varchar(255), name varchar(255), episode integer, season integer, transmission_id integer, transmission_name varchar(255), download_dir varchar(255), renamed boolean default false, foreign key (tmdb_id) references shows(tmdb_id) on delete set null);
